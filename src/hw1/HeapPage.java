...
public class HeapPage {
	...
	public int getNumSlots() {
		//your code here
		// page.size in byte * 8 / (record.size in byte * 8 + 1)
		
//		int ans = (HeapFile.PAGE_SIZE - getHeaderSize()) / this.td.getSize();
//		return (HeapFile.PAGE_SIZE - getHeaderSize()) / this.td.getSize();
		float ans = HeapFile.PAGE_SIZE / (td.getSize() + 1/8.0f);
		return (int) ans;
	}
	...
	private int getHeaderSize() {        
		//your code here
//		return HeapFile.PAGE_SIZE%(1+this.td.getSize()*8)==0 ? size:size+1 ;
		int size = getNumSlots() / 8;
		return getNumSlots()%8 == 0 ? size : size + 1;
	}
	...
	public boolean slotOccupied(int s) {
		...
		int d = s/8;
		int m = s%8;
		return ((header[d] >> m) & 1) == 1 ? true : false;
	...
	public void setSlotOccupied(int s, boolean value) {
		...
		int d = s/8;
		int m = s%8;
		if(value) header[d] = (byte)(header[d] | (1 << m));
		else header[d] = (byte)(header[d] & ~ (1 << m));
	}
	...
	public void addTuple(Tuple t) throws Exception {
		//your code here
		for(int i=0; i<header.length; i++) {
			if(header[i] == 0xFF) continue;
			int ind = 0;
			while(ind < 8 && ind + i*8 < getNumSlots()) {
				int curTupleInd = ind + i*8;
				if(!slotOccupied(curTupleInd)) {
					setSlotOccupied(curTupleInd, true);
					tuples[curTupleInd] = t;
					t.setId(curTupleInd);
					t.setPid(id);
					return ;
				}
	...
	public void deleteTuple(Tuple t) throws Exception {
		//your code here
		if(t.getPid() != getId() || tuples.length == 0) throw new Exception();
//		int ind=0;
//		while(ind < tuples.length) {
//			if(t.getId() == tuples[ind].getId()) {
//				break;
//			}
//			ind++;
//		}
	...
	public class TupleIterator implements Iterator<Tuple> {
		...
		public boolean hasNext() {
			while(nextInd < tuples.length) {
				if(tuples[nextInd] != null) return true;
				nextInd++;
			}
			return false;
		}
